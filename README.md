...
- 모니터링페이지 데이터 변경 연동
    ...
    - 방법 2 : 5초마다 setState 사용

    - 현재 방법2를 사용하여 구현하였습니다
        
        - 다만 방법1이 더 좋은 방법이라 생각해서 현재 구현중이지만 아직 완성을 못했습니다.

        - 따라서 지금은 방법2를 사용하였습니다.
...
3. monitoringPage.dart
    ...
    - 데이터는 (일단은) 블루투스 데이터 통신으로 값을 받아옴
        ...
        - 기존에는 버튼 클릭시 차량 데이터가 초기화가 안되었다면 차량으로 부터 데이터를 받아옴

            - 이 방법에서 문제점은 5초마다 데이터가 갱신되는 앱 특성상 충돌 위험이 있음

            - 일단은 블루투스를 연결하고나서부터 5초 뒤 데이터를 얻기 시작함

            - 그렇기 때문에 블루투스 연결을 성공하면 임시로 3초간 로딩시간을 두기로 결정

            - 이는 임시이며 더 좋은 방법을 찾을 예정

    - 이슈(해결됨) : 1분마다 데이터가 변경되는데 문제는 모니터링 페이지에 계속 머물시 데이터가 변경되더라도 화면 출력 값은 그대로

        - 방법 1 : provider 사용

        - 방법 2 : 5초마다 setState 사용

        - 방법 1을 시도했으나 문제가 계속 발생해 방법 2로 변경
